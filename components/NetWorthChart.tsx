...
const NetWorthChart: React.FC<NetWorthChartProps> = ({ data, className }) => {
  if (!data || data.length < 2) {
    return (
      <div className={`flex items-center justify-center h-48 bg-gray-900/50 rounded-lg text-gray-400 ${className}`}>
        Not enough data to display net worth history.
      </div>
    );
  }

  const isGrowth = data[data.length - 1].netWorth >= data[0].netWorth;
  const strokeColor = isGrowth ? '#4ade80' : '#f87171';
  const gradientColor = isGrowth ? '#4ade80' : '#f87171';
  
  // Use a unique ID for the gradient per chart instance
  const gradientId = useMemo(() => `networth-gradient-${Math.random().toString(36).substring(2, 9)}`, []);
  const filterId = useMemo(() => `networth-glow-${Math.random().toString(36).substring(2, 9)}`, []);
